...
struct MainWindow {
  static let windowTitle = "Imprimerie v0.2 (b1)"    // Window title
  ...
  static let predicateMonth = Date().monthToString()    // Month to string
  static let predicateYear = Date().yearToString()    // Current string yerar
...
private func getNbJoursTravailles() -> Int {
  ...
  for jourTravaille in joursTravailles {
    if jourTravaille.annee == MainWindow.predicateYear {
...
private func getSalaireJournalier() -> Double {
  ...
  for masseSalariale in massesSalariales {
    if masseSalariale.annee == MainWindow.predicateYear {
